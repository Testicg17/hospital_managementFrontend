...
function PatientPortal() {
    ...
    <html>
      <head>
        ...
        <style>
          ...
          .med-table th { background: #f0fdf4; color: #166534; border: 1px solid #bbf7d0; padding: 10px; text-align: left; font-size: 12px; text-transform: uppercase; }
  ...
  const handleAppointmentRequest = async (appointmentData) => {
    const scheduleError = getScheduleValidationMessage(appointmentData.date, appointmentData.time);
    if (scheduleError) {
      alert(scheduleError);
      return;
    }
  ...
  const handleRescheduleRequest = async (appointmentId, requestedDate, requestedTime, reason, requestedHospitalLocationId) => {
    const scheduleError = getScheduleValidationMessage(requestedDate, requestedTime);
    if (scheduleError) {
      alert(scheduleError);
      return;
    }
                ...
                {timeOptions.length > 0 ? (
                  <select required value={formData.time} onChange={(e) => setFormData({ ...formData, time: e.target.value })}
                    ...
                    {timeOptions.map((time) => {
                      ...
                      return <option key={time} value={time} disabled={outsideWindow}>{time}{outsideWindow ? ' - not available' : ''}</option>;
                    })}
                  </select>
                ) : (
            ...
            <div>
              ...
              {timeOptions.length > 0 ? (
                ...
                >
                  <option value="">Select time...</option>
                  {timeOptions.map((time) => {
                    const outsideWindow = !isScheduleSlotAllowed(requestedDate, time);
                    return <option key={time} value={time} disabled={outsideWindow}>{time}{outsideWindow ? ' - not available' : ''}</option>;
                  })}
                </select>
              ) : (
