 ...
 *  renders as inline SVG rectangles, and that downloadQRPng
 *  rasterizes onto a <canvas> for PNG export.
 *
 *  Exports:
 *    - QRCodeSVG(props)              React component, inline SVG QR code
 *    - downloadQRPng(value, ...)     Client-side PNG export/download
 *    - qrEncode(text, level)         Low-level: text -> boolean matrix
 * ============================================================
 */
...
export function qrEncode(text, errorCorrectLevel = QRErrorCorrectLevel.M) {
 ...
 */
export function downloadQRPng(value, filename = "qr-code.png", pixelSize = 1024, quietZone = 2, fgColor = "#16241f", bgColor = "#ffffff") {
...
export function QRCodeSVG({ value, size = 96, quietZone = 2, fgColor = "#16241f", bgColor = "#ffffff", className }) {
